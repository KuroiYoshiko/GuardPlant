...
class DiscoverViewModel(private val repository: SpeciesRepository) : ViewModel() {
    ...
    val uiState = combine(repository.species, repository.favouriteSpeciesIds, query, filters) { species, favourites, search, activeFilters ->
        val filtered = species.filter { item ->
            (search.isBlank() || item.scientificName.contains(search, true) || item.commonNames.any { it.contains(search, true) }) &&
                (activeFilters.light == null || item.light == activeFilters.light) &&
                (activeFilters.difficulty == null || item.difficulty == activeFilters.difficulty) &&
                (activeFilters.toxicToPets == null || item.toxicToPets == activeFilters.toxicToPets) &&
                (activeFilters.growthHabit == null || item.growthHabit == activeFilters.growthHabit) &&
                (activeFilters.humidity == null || item.humidity == activeFilters.humidity) &&
                (activeFilters.matureSize == null || item.matureSize == activeFilters.matureSize)
        }
...
fun DiscoverRoute(repository: SpeciesRepository, onSpeciesClick: (String) -> Unit) {
    ...
    LazyColumn(
        ...
        item { ScreenHeader("Discover", "A curated offline catalogue preview") }
        ...
        if (state.species.isEmpty()) {
            item { EmptyState("No matches", "Try clearing a filter or searching for another plant.", "Clear filters", viewModel::clearFilters) }
        } else {
...
private fun FilterSheet(filters: SpeciesFilters, onFiltersChanged: (SpeciesFilters) -> Unit, onDismiss: () -> Unit) {
    ModalBottomSheet(onDismissRequest = onDismiss) {
        Column(Modifier.padding(horizontal = 20.dp).padding(bottom = 32.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
            ...
            FilterGroup("Light", LightLevel.entries, filters.light, { it.label }) { onFiltersChanged(filters.copy(light = it)) }
            FilterGroup("Difficulty", (1..5).toList(), filters.difficulty, { "Level $it" }) { onFiltersChanged(filters.copy(difficulty = it)) }
            FilterGroup("Pet safety", listOf(false, true), filters.toxicToPets, { if (it) "Toxic" else "Pet safe" }) { onFiltersChanged(filters.copy(toxicToPets = it)) }
            FilterGroup("Growth habit", GrowthHabit.entries, filters.growthHabit, { it.label }) { onFiltersChanged(filters.copy(growthHabit = it)) }
            FilterGroup("Humidity", HumidityLevel.entries, filters.humidity, { it.label }) { onFiltersChanged(filters.copy(humidity = it)) }
            FilterGroup("Mature size", MatureSize.entries, filters.matureSize, { it.label }) { onFiltersChanged(filters.copy(matureSize = it)) }
...
@Composable
private fun <T> FilterGroup(title: String, options: List<T>, selected: T?, label: (T) -> String, onSelect: (T?) -> Unit) {
